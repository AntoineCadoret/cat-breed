...
const Modal = ({ fact, modalCallback, visible }, props) => {
  const unShow = () => {
    //  Function telling the CardList component to hide the modal
    modalCallback();
  };
  return (
    // modal showing a random fact about cat
    <div className={visible ? "block" : "hidden"}>
      <div
        className="bg-black/50 w-full h-full fixed top-0 right-0 left-0 z-300"
        onClick={() => unShow()}
      >
        <div className="modal rounded-md bg-gray-200 fixed top-0 right-0 left-0 z-500 w-1/3 m-auto md:inset-0 h-modal md:h-1/3 flex flex-col">
          <div className="flex flex-row justify-between bg-slate-800 text-white font-bold">
            <h4 className="p-4">A random fact about Cats</h4>
